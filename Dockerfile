...
if [ "${TARGETPLATFORM}" != "linux/arm/v6" ] && [ "${TARGETPLATFORM}" != "linux/386" ] \
    && [ "${TARGETPLATFORM}" != "linux/s390x" ]; then
    ...
    echo "Onnxruntime Builder does not currently support ${TARGETPLATFORM}"
fi
rm -rf /var/cache/apk/*
rm -rf /usr/share/man/*
EOT
